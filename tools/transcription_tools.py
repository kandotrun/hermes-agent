...
def _transcribe_local_command(file_path: str, model_name: str, context: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-local-stt-") as output_dir:
            ...
            if use_shell:
                subprocess.run(command, shell=True, check=True, capture_output=True, text=True, timeout=300, stdin=subprocess.DEVNULL, env=command_env, creationflags=windows_hide_flags())
            else:
                subprocess.run(shlex.split(command), check=True, capture_output=True, text=True, timeout=300, stdin=subprocess.DEVNULL, env=command_env, creationflags=windows_hide_flags())
